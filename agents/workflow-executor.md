...
2. **Plan file path** — absolute path to the approved implementation plan (the `planning-report.md` of this run, inside the session's run directory). Its body contains the plan content; the YAML frontmatter at the top (`epoch:` / `result:`) is state-machine metadata that you can ignore for content reading.
