# 用 meta-workflow 驯服 Claude Code

一本 cookbook：列举 Claude Code 常见的"不听话"场景、直接用 prompt 讲为什么不够用、以及用 `meta-workflow` 插件把规矩写成状态机的做法。
...
`meta-workflow` 把这些规矩写成 `workflow.json` 状态机：每个阶段有输入依赖、产出 artifact、pass/fail 判据。偏离了状态机会把它拉回来；到了 `max_epoch` 强制 `escalated`，不会无限打转。
...
/meta-workflow:create-workflow "<自然语言描述>"

# 用创建好的 workflow 跑一个任务
/meta-workflow:start --workflow=cloud://<你>/<名字> "<任务>"
```
...
**写成 workflow**：
```
/meta-workflow:create-workflow "研究优先工作流。
...
发布后跑：
```
/meta-workflow:start --workflow=cloud://you/research-first "给 API 加限流"
```
...
**写成 workflow**：
```
/meta-workflow:create-workflow "execute → build → test → review 四段式。
- execute：写代码。
...
**写成 workflow**：
```
/meta-workflow:create-workflow "最小改动工作流。
...
**写成 workflow**：
```
/meta-workflow:create-workflow "根因分析工作流。
...
**写成 workflow**：
```
/meta-workflow:create-workflow "TDD 严格模式。
...
**写成 workflow**：
```
/meta-workflow:create-workflow "UI 改动工作流。
- execute：改代码。
...
```
/meta-workflow:create-workflow "... 每个 stage 的 instructions 里都显式列：
...
- **改已有 workflow**：`/meta-workflow:create-workflow --workflow=cloud://you/name "在 qa 后加一个 deploy-dry-run 阶段"` —— 同一个命令既是创建也是编辑。
- **逛别人的 workflow**：<https://workflows.worldstatelabs.com/hub>。
- **中途介入**：`/meta-workflow:interrupt` 暂停，`/meta-workflow:continue` 恢复（支持跨机器，需要 git push 同步代码）。
- **本地模式**：加 `--mode=local` 全离线，state 存在 `<project>/.meta-workflow/`。
