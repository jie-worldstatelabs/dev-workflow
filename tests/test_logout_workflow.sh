...
mkdir -p "$FAKE_HOME2/.config/stagent"
cat > "$FAKE_HOME2/.config/stagent/auth.json" <<'EOF'
{
...
[[ ! -f "$FAKE_HOME2/.config/stagent/auth.json" ]]
...
mkdir -p "$FAKE_HOME3/.config/stagent"
cat > "$FAKE_HOME3/.config/stagent/auth.json" <<'EOF'
{
...
mkdir -p "$FAKE_HOME4/.config/stagent"
echo '{"label": "anon-device"}' > "$FAKE_HOME4/.config/stagent/auth.json"
...
mkdir -p "$FAKE_HOME5/.config/stagent"
echo '{"user_id": "demo:1"}' > "$FAKE_HOME5/.config/stagent/auth.json"
