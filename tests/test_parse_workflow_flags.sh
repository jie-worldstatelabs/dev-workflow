...
run_ok "A3" "cloud://author/name accepted" \
  "--flow=cloud://demo/my-workflow do the thing" \
  "WORKFLOW_FLAG=cloud://demo/my-workflow" \
  "WF_TYPE=cloud" \
  "MODE=cloud"

# A4: cloud ref + --mode=cloud → ok
run_ok "A4" "--flow=cloud://... --mode=cloud → ok" \
  "--flow=cloud://demo/wf --mode=cloud run it" \
...
run_err "A5" "cloud:// + --mode=local → error" \
  "--flow=cloud://demo/wf --mode=local do it"

# A6: nonexistent local path → error
run_err "A6" "nonexistent local path → error" \
  "--flow=/this/path/does/not/exist/workflow do it"

# A7: bare unrecognised value (no cloud://, no path prefix) → error
run_err "A7" "bare unrecognised --flow value → error" \
  "--flow=not-a-valid-value do it"

# A8: --mode=local without --flow → ok (no path to validate)
...
run_ok "A9" "description is residual text after flags stripped" \
  "--flow=cloud://demo/wf my task description" \
  "DESCRIPTION=my task description"

# A10: space-separated --flow <value> form (legacy)
run_ok "A10" "--flow <value> space-separated form accepted" \
  "--flow cloud://demo/legacy-wf some task" \
...
run_ok "A11" "cloud://author/name with dots and dashes" \
  "--flow=cloud://my-org/my.workflow-v2 task" \
...
run_ok "A13" "local dir + --mode=cloud → ok (allowed)" \
  "--flow=${_SMOKE_WF} --mode=cloud do it" \
